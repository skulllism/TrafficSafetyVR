...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenTransformLocalJump : FsmStateAction
    {
        ...
        public override void OnEnter()
        {
            ...
            switch (tweenIdType)
            {
                case DOTweenActionsEnums.TweenId.UseString: if (string.IsNullOrEmpty(stringAsId.Value) == false) sequence.SetId(stringAsId.Value); break;
                case DOTweenActionsEnums.TweenId.UseTag: if (string.IsNullOrEmpty(tagAsId.Value) == false) sequence.SetId(tagAsId.Value); break;
                case DOTweenActionsEnums.TweenId.UseGameObject: sequence.SetId(Fsm.GetOwnerDefaultTarget(gameObject)); break;
            }
            ...
            switch (selectedEase)
            {
                case DOTweenActionsEnums.SelectedEase.EaseType: sequence.SetEase(easeType); break;
                case DOTweenActionsEnums.SelectedEase.AnimationCurve: sequence.SetEase(animationCurve.curve); break;
            }
            ...
            if (debugThis.Value) Debug.Log("GameObject [" + State.Fsm.GameObjectName + "] FSM [" + State.Fsm.Name + "] State [" + State.Name + "] - DOTween Transform Local Jump - SUCCESS!");
