...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenTransformLocalMoveY : FsmStateAction
    {
        ...
        {
            tweener = Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<Transform>().DOLocalMoveY(to.Value, duration.Value, snapping.Value);
            ...
            if (playInReverse.Value) tweener.From(setReverseRelative.Value);
            ...
            if (debugThis.Value) Debug.Log("GameObject [" + State.Fsm.GameObjectName + "] FSM [" + State.Fsm.Name + "] State [" + State.Name + "] - DOTween Transform Local Move Y - SUCCESS!");
